...
next VGA frame never arrives. A live stopped text-mode guest proof found the initial
...
An execution breakpoint condition is the structured object
`{register, operator, value}`. Register names are the lowercase names advertised
by capabilities, operators are `eq|ne`, and value is fixed-width
`0xNNNNNNNN`. It is evaluated before the instruction. `hit_filter={skip,every}`
counts only condition matches, ignores `skip`, then selects the first eligible
